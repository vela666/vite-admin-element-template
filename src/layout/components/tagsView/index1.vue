<template>
  <div
    ...
    <el-scrollbar
      ...
      <router-link
        v-for="(tag, index) of routesVisited"
...
function initTags() {
  const affixs = (affixTags.value = filterAffixTags(routes.value))
...
onBeforeUnmount(() => {
  scrollWrapper.value &&
    scrollWrapper.value.removeEventListener('scroll', handleScroll, true)
})
...
.tags-view-wrapper {
  ...
  .#{$customNameSpace}-scrollbar__wrap {
    height: 49px;
  }*/
  .tags-view-item {
