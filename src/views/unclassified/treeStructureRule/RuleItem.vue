<template>
  ...
  <div v-else class="rulers-select-wraper">
    ...
    <div class="add-item">
      <span class="horizontal-line"></span>
      <i class="ml-5 i-icon ri-add-circle-fill" @click="insertRuleItem(null)"
        >+</i
      >
    </div>
...
<script setup>
import { reactive, toRefs, nextTick, watch, ref } from 'vue'
 ...
 */
function insertRuleItem(ruleItem) {
  if (!ruleItem) {
 ...
 * 移除规则节点
 */
function removeRuleItem(ruleItem) {
  emit('removeRuleNode', ruleItem)
}

function removeRuleNode(node) {
